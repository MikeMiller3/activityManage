<template>
	<div class="industryCategory-cmp">
		<div class="clearfix">
			<p class="scene-title el-c-info m-l-15">行业分类</p>
		...
		<el-collapse v-if="tableData.length > 0">
			<el-collapse-item class="list-con" v-for="item in tableData" :key="item.id">
				...
				<el-collapse v-if="item.childrenList && item.childrenList.length > 0">
					<el-collapse-item class="list-con" v-for="item in item.childrenList" :key="item.id">
						<template slot="title">
							<div class="table-title pull-right">
								<div class="scene-tables-left">
									<span class="table-title-left">{{item.name}}</span>
							...
							<el-collapse-item class="list-con" v-for="item in item.childrenList" :key="item.id">
								<template slot="title">
									<div class="table-title pull-right">
										...
										<div class="scene-tables-right" @click.stop>
											<span class="el-c-primary" @click="dialogVisible = true">添加</span>
											<span class="el-c-primary"
												  @click="updateName(item.id,item.name)">修改名称</span>
											<span class="el-c-primary" @click="del">删除</span>
											<span class="el-c-primary" @click="edit(item.id,item.name)">参数配置</span>
										</div>
									</div>
...
<style>
	.industryCategory-cmp .el-collapse-item__content {
		padding: 0 15px;
	}
</style>
